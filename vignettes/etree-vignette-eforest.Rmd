...
This vignette explains how these two models can be used for classification and 
...
arguments. Here, we also add <tt>ntrees = 10</tt> to reduce the computational
burden.
...
regression, it uses the Root Mean Square Percentage Error as performance metric,
and <tt>random_covs</tt> is set to one third (rounded down) of the total number
of covariates.
...
only. Here, we also set <tt>ntrees = 10</tt> to reduce the computational burden 
...
Also predictions are obtained in the same way as for regression, but we cover
this part also for classification for the sake of completeness. Note that, for
classification, individual trees' predictions for single observations are 
aggregated by majority voting rule.
...
In this case, we obtain a misclassification error of 0.4 on test data, which 
